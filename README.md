...
The threshold is five files, or a hundred and fifty changed lines, or a single file whose
...
commands the hook saw and how they exited, `plan` if there is one, `task` as the first
prompt of the session read from the transcript. Two sections are left for the model,
`decisions` and `next_action`, and one line asks for them at the threshold:
