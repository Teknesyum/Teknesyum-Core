...
`dotnet build` count as proof. The proof is tied to the files the model wrote, so a release
...
| `scripts/log.js write` | A log with a fixed shape into Core's `logs/openlogs/`: `--kind hata` (a bug, the default), `yontem` (a method worth keeping) or `teklif` (a proposal). A prompt such as `core'a raporla` or `report this to core` brings the recipe into that turn only. |
...
```powershell
irm https://raw.githubusercontent.com/Teknesyum/Teknesyum-Core/v0.46.0/install.ps1 | iex
```

### macOS / Linux - one line

```bash
curl -fsSL https://raw.githubusercontent.com/Teknesyum/Teknesyum-Core/v0.46.0/install.sh | bash
```
...
```bash
git clone --depth 1 --branch v0.46.0 https://github.com/Teknesyum/Teknesyum-Core "$HOME/Teknesyum-Core"
```
...
| Event | Hook | Says |
|---|---|---|
| `SessionStart` | `count.js` | `Resume: .claude/handoff.md` if one exists; the first open `- [ ]` step of `docs/plan.md` if one exists; the trash offer when `<project>/trash` is over 100 MB, at most once a day per project — the line carries the exact emptying command and the hook never empties it itself; the open lines of `.claude/acik.md` on every source, compact included; after a compaction, `Reply language: Turkish.` when the owner's `CLAUDE.md` asks for Turkish; else nothing. Once a day it also starts `kutuphane.js fetch all --stale 7` detached in the background, so no shelf is older than a week, and runs the `cop.js` sweep of week-old state files and old plugin versions; the model sees none of it |
| `UserPromptSubmit` | `mod.js` | library hits on `??` / `++`, private books on `pp`, agency seats on `aa`, the consult recipe on `ff`, the memory-check recipe on `mc`, the UI-check recipe on `uc` (the `ui-denetim.md` shelf book, the teknesyum-ui scanner and live contrast snippet), the list of marks on `hh` (that list goes to the display channel, not into the context) — the mark is read at the start or at the end of the prompt, and `mc` at the start counts only alone or before a scope such as `2 hafta`; the open lines of `.claude/jobs.md` (the job list, `- [ ] job — reason`) and of an old `.claude/sonra.md` move into the ledger `.claude/acik.md` and the file moves to `trash/`, `[x]` ledger lines are pruned to `trash/`, and the open ledger lines ride along; a prompt that asks for a Core report (`core'a raporla`, `teknesyum'a bildir`, `report this to core`) gets the `log.js write` recipe; a prompt with several items leaves a state marker, no context; else nothing |
...
| `PreToolUse` | `yasak.js` | a denied command with one line on what to do instead. Deleting inside the project is free; leaving it is not — a delete whose target resolves outside the working directory, or is the root itself, is denied, along with disk writes, history rewrites, repo and release deletion, download-and-run pipes, `chmod 777` and machine-wide kills. A history rewrite or a repo, release or remote-branch delete passes once the owner's last message says yes (`evet`, `onay…`, `sil…`, `kaldır…`, `yes`), so the model runs it itself instead of handing over the command; disk writes, root wipes and kills never pass; then, in the same process, the loop gate: one line when a wait loop has no upper bound; else nothing |
...
| `Stop` | `dur.js` | the interface gate: when this turn edited a `.tsx` `.jsx` `.vue` `.svelte` `.css` `.scss` `.less` `.html` `.axaml` or `.xaml` file and no screenshot followed the last such edit, the turn is held once and asked for a capture at the smallest size, an outside look by a subagent that did not do the work, and "it works" and "it is usable" as two claims; in a desktop project (`src-tauri/`, Electron, XAML) a browser screenshot does not count. Off: `ui: false`. Then, in the same process, when the reply ends with a `Kullanılan kitaplar:` / `Books used:` line, it compares that line with the books really read and queues `Books Differ · Said … · Read …` when they differ; the count Stop work: refreshes the diff, and after `agency.js show` prints the seat once as a chat line. Then a session that edited files and ran nothing is blocked once; the proof is a hash of the files the model wrote, a turn with no Write or Edit is never asked, and a block is not repeated. Off with `evidence: false`. A reply of 25 words or more that reads as English while the owner's reply language is Turkish is held once. Off with `langCheck: false`. `Kullanılan kitaplar: yok` declares no books. The job gate shares the block: an open line in `.claude/jobs.md` with no reason, a list-shaped prompt with no list or a shorter list (messages queued while the turn ran each add at least one job), or a reply of 40 characters or less while `.claude/acik.md` has open lines, holds the turn once. Off with `jobs: false` |
| `SessionEnd` | `handoff.js` | nothing; writes the handoff |
| `Notification` | `notify.js` | nothing; rings |
| `MessageDisplay` | `bant.js` | nothing; draws the queued `Teknesyum Core > …` lines on screen only. On the last chunk of a message it reads the turn from the transcript and adds `Books Used · …` under the reply: skills called, files read from the private shelf, the library or a skill folder, and `kutuphane.js show` slugs; a turn with no book draws nothing, the same list is not drawn twice |
