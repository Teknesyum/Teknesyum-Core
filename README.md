...
| `scripts/hatirla.js` | Memory check: `topla` pages my past requests out of the project's transcripts, 60 per page and newest first, into `tmp/gecmis-N.md`, with the open lines of old job lists on page one. The session model reads page by page and stops when older pages show only settled work - there is no fixed limit and no need to reach the first record. A subagent names what was never finished and `record --reply <file>` files that list as `tmp/hatirlatici.md`. Temporary files live in `tmp/`, which stays out of git. |
