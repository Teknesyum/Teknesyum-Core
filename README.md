...
| `scripts/hatirla.js` | Memory check: `topla [--gun N]` writes every past request, whole, with its evidence - the closing reply, the commits made before the next request, the open job lines of `trash/jobs-*`, `.claude/jobs.md` and `docs/plan.md` - into pages of about 40k characters under `tmp/gecmis-N.md`, newest first, a repeated request kept once. It prints only an index (page, dates, requests, unclosed), the session model picks the depth from it and hands the page paths to sonnet. `record --ajan <agentId>` files the reply as `tmp/hatirlatici.md` in three sections - open, awaiting decision, unclear - and puts one count line on screen. Temporary files live in `tmp/`, which stays out of git. |
...
| `UserPromptSubmit` | `mod.js` | library hits on `??` / `++`, private books on `pp`, agency seats on `aa`, the consult recipe on `ff`, the memory-check recipe on `mc`, the list of marks on `hh` (that list goes to the display channel, not into the context) — the mark is read at the start or at the end of the prompt, and `hh` / `mc` at the start count only alone or before a scope such as `2 hafta`; the open lines of `.claude/jobs.md` (the job list, `- [ ] job — reason`) come back once and the file moves to `trash/`; a prompt with several items leaves a state marker, no context; else nothing |
