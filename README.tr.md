...
| `scripts/hatirla.js` | Bellek taraması: `topla` geçmiş isteklerimi projenin konuşma kayıtlarından sayfa başına 60, yeniden eskiye `tmp/gecmis-N.md` dosyalarına süzer; eski iş listelerinin açık satırları ilk sayfadadır. Oturum modeli sayfa sayfa okur, eski sayfalar yalnız kapanmış iş gösterince durur — sabit sınır yok, ilk kayda varmak şart değil. Bitmemiş olanları bir alt ajan adlandırır, `record --reply <dosya>` o listeyi `tmp/hatirlatici.md` olarak dosyalar. Geçici dosyalar `tmp/` altında durur, git'e girmez. |
