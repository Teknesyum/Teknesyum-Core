...
| `scripts/hatirla.js` | Bellek taraması: `topla [--gun N]` geçmiş her isteği kesmeden kanıtıyla — kapanış cevabı, sonraki isteğe kadarki commit'ler, `trash/jobs-*`, `.claude/jobs.md` ve `docs/plan.md` açık satırları — yaklaşık 40 bin karakterlik `tmp/gecmis-N.md` sayfalarına yeniden eskiye yazar, tekrarlanan istek bir kez. Yalnız dizin basar (sayfa, tarih, istek, kapanışsız); oturum modeli derinliği dizinden seçer, sayfa yollarını sonnet'e verir. `record --ajan <agentId>` cevabı `tmp/hatirlatici.md` olarak üç bölümde — açık, kararını bekliyor, belirsiz — dosyalar ve ekrana tek sayım satırı basar. Geçici dosyalar `tmp/` altında durur, git'e girmez. |
...
```powershell
irm https://raw.githubusercontent.com/Teknesyum/Teknesyum-Core/v0.36.1/install.ps1 | iex
```

### macOS / Linux - tek satır

```bash
curl -fsSL https://raw.githubusercontent.com/Teknesyum/Teknesyum-Core/v0.36.1/install.sh | bash
```
...
```bash
git clone --depth 1 --branch v0.36.1 https://github.com/Teknesyum/Teknesyum-Core "$HOME/Teknesyum-Core"
```
...
| `UserPromptSubmit` | `mod.js` | `??` / `++`de kütüphane bulguları, `pp`de özel kitaplar, `aa`da ajans koltukları, `ff`de fable danışma yordamı, `mc`de bellek tarama yordamı, `hh`de işaretlerin listesi (liste bağlama değil ekran kanalına gider) — işaret cümlenin başında da sonunda da okunur, `hh` / `mc` başta yalnız tek başına ya da `2 hafta` gibi kapsamla sayılır; `.claude/jobs.md`'nin (iş listesi, `- [ ] iş — gerekçe`) açık satırları bir kez geri gelir, dosya `trash/`'e taşınır; çok maddeli istem bağlama yazmadan durum işareti bırakır; yoksa hiçbir şey |
