...
function emit(root, graph) {
  ...
  const json = { _map: { head: head, at: new Date().toISOString() } };
  for (const [f, n] of all)
...
function who(root, target) {
  ...
  try {
    ...
    process.stdout.write('No map yet. Run: node <plugin>/scripts/map.js .\n');
    return 1;
  }
  const want = String(target).replace(/\\/g, '/').replace(/^\.\//, '');
  const key = Object.keys(json).find((k) => k === want || k.endsWith('/' + want));
  if (!key) {
    process.stdout.write(want + ' is not in the map.\n');
    return 1;
  }
  const node = json[key];
  const from = node.from || [];
  const st = staleness(root, dir);
  process.stdout.write(
    key + ' (' + node.lines + 'L)\n' +
      (from.length ? 'imported by:\n' + from.map((f) => '  ' + f).join('\n') + '\n' : 'nothing imports it\n') +
      (node.to && node.to.length ? 'imports:\n' + node.to.map((f) => '  ' + f).join('\n') + '\n' : '') +
      (st.state === 'stale'
        ? '\nthe map is ' + (st.behind === null ? 'behind' : st.behind + ' commits stale') + ' - run map.js again\n'
        : '')
  );
  return 0;
}
...
module.exports = { build, emit, scan, cycles, staleness, headOf, who };
