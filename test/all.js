...
function testJobs() {
  ...
  ok('the open job lands in the ledger with its reason', /^- \[ \] issue 2 şablonu — \d{4}-\d\d-\d\d \d\d:\d\d — sahip kararı$/m.test(fs.readFileSync(book, 'utf8')), fs.readFileSync(book, 'utf8'));
  const again = mod.handle({ hook_event_name: 'UserPromptSubmit', prompt: 'merhaba', cwd });
  ok('the ledger comes back every turn while a line is open', /issue 2 şablonu/.test(again) && /acik\.md/.test(again), again);
  fs.writeFileSync(list, '- [ ] issue 2 şablonu — yine\n- [x] a');
  mod.handle({ hook_event_name: 'UserPromptSubmit', prompt: 'merhaba', cwd });
  ok('the same job is not written twice', fs.readFileSync(book, 'utf8').split('issue 2 şablonu').length === 2, fs.readFileSync(book, 'utf8'));
  fs.writeFileSync(book, fs.readFileSync(book, 'utf8').replace('- [ ]', '- [x]'));
  ok('a ticked ledger costs nothing and leaves for trash', mod.handle({ hook_event_name: 'UserPromptSubmit', prompt: 'merhaba', cwd }) === '' && !fs.existsSync(book) && fs.readdirSync(path.join(cwd, 'trash')).some((n) => /^acik-.*\.md$/.test(n)));
  ...
  fs.writeFileSync(path.join(cwd, '.claude', 'sonra.md'), '# Sonra\n\n- eski erteleme\n');
  const moved = mod.handle({ hook_event_name: 'UserPromptSubmit', prompt: 'merhaba', cwd });
  ok('sonra.md moves into the ledger once and leaves for trash', /eski erteleme — .* — sonra\.md/.test(moved) && !fs.existsSync(path.join(cwd, '.claude', 'sonra.md')) && fs.readdirSync(path.join(cwd, 'trash')).some((n) => /^sonra-.*\.md$/.test(n)), moved);
  fs.unlinkSync(book);
  ...
  try {
    ...
    process.env.TEKNESYUM_NO_REFRESH = '1';
    const opened = count.handle({ hook_event_name: 'SessionStart', source: 'compact', session_id: 'j7', cwd });
    delete process.env.TEKNESYUM_NO_REFRESH;
    ok('a compact brings the ledger back', /bekleyen/.test(opened) && /ajan: Ölçüm/.test(opened), opened);
    fs.unlinkSync(book);
